...
/*   Updated: 2023/03/08 18:42:56 by thepaqui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_cursor_pos(t_game *game)
{
	...
	if (game->curs_pos.x < SPR_DIM)
		game->curs_pos.x = SPR_DIM;
	else if (game->curs_pos.x > game->win_size.x - 1 - (SPR_DIM * 2))
		game->curs_pos.x = game->win_size.x - 1 - (SPR_DIM * 2);
	if (game->curs_pos.y < SPR_DIM)
		game->curs_pos.y = SPR_DIM;
	else if (game->curs_pos.y > game->win_size.y - 1 - (SPR_DIM * 2))
		game->curs_pos.y = game->win_size.y - 1 - (SPR_DIM * 2);
}
...
void	update_cursor(t_game *game)
{
	...
	{
		approx_pro_pos.x = nearbyint(game->player->pro_pos.x);
		approx_pro_pos.y = nearbyint(game->player->pro_pos.y);
		refresh_area(game, approx_pro_pos, 2, 2);
	}
