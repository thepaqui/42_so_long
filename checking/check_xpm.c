...
/*   Updated: 2023/02/11 16:22:11 by thepaqui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_properties(char *is, int should, int *pal_size)
{
	char	**tab;
	int		err;
	...
	if (!tab)
		return (MALLOCFAIL);
	err = 0;
	if (!ft_atoi(tab[0]) || !ft_atoi(tab[1])
	...
	if (!err)
	{
		*pal_size = ft_atoi(tab[2]);
		err = check_palette_basic(is, pal_size);
	}
	ft_free_tab(tab, -1);
	return (err);
}
...
int	check_xpm(char **xpm, t_xpm *stru)
{
	...
	if (ft_strncmp(xpm[4], "* pixels *", 10))
		return (BADXPM);
	err = check_properties(xpm[3], SPRITE_DIM, &(stru->pal_size));
	return (err);
}